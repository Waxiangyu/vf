...
define(function () {
    ...
    var filter={
        ...
        dialogTemplate='<div class="modal-dialog"><div class="modal-dialog-main center" $-class="{fullscreen:innerConf.zoom == \'max\'}" $-parses:style="height:{{innerConf.height}};width:{{innerConf.width}}">' +
            '<div class="dialog-header"><div class="header-conent">' +
            '<strong class="title-name">{{innerConf.title}}</strong>' +
            '<span class="title-btns"><i class="iconfont icon-fangda" $-if="innerConf.maxmin" $-on:click="interface.zoomToggle"></i><i class="iconfont icon-close" $-on:click="interface.close"></i></span>' +
            '</div></div>' +
            '<div class="dialog-body"><div class="body-content" $-parses:style="width:{{innerConf.width}}">' +
            '<template config="innerConf|templateHandle:[$,innerConf.content]"></template>' +
            '</div></div>' +
            '<div $-if="innerConf.btns|btnLen"  class="dialog-footer">' +
            '<ul class="footer-content"><li $-for="btnInfo in innerConf.btns"><button type="button" $-bind:class="btnInfo.theme" $-style="btnInfo.style" $-on:click="btnInfo|trigger:[$,interface,innerConf]">{{btnInfo.name}}</button></li></ul>' +
            '</div>' +
            '</div></div>';
    ...
    return function dialogRender(conf) {
        ...
        var scope={
            ...
            dialogEle=scope.ele.dialogEle=$FRAME.viewVM(dialogTemplate,scope,filter).firstChild;
