 ...
 */
formLayoutEdit(function ($app, $appConfig) {

    //表单布局
    var layoutHTML = '<div class="form-layout-edit"><ul><li $-class="{required:eleConf.required,hidden:eleConf.hidden}" $-drag="eleConf.dragConf"  $-for="(index,eleConf) in layoutConfig.list" $-bind:class="eleConf.class" $-cmd="eleConf"><template config="eleConf|rowTemplateHandle"></template><ul class="actions"><li $-for="action in eleConf.actions" $-bind:title="action.name" $-on:click="action.trigger|actionHandle:[$,action,layoutConfig,index,eleConf]"><span><i $-parses:style="color:{{action.color}}" class="iconfont" $-bind:class="action.icon"></i></span></li></ul></li></ul></div>',
        ...
        filter={
            actionHandle:function (trigger,action,layoutConfig,index,eleConf) {
                return function (e) {
                    trigger.apply(action,[e,{
                        index:index,
                        rowData:eleConf,
    ...
    //form-layout组件注册
    $app.componentRegister('formLayoutEdit', {
        //指令优先级 降序执行
        priority: 0,
        //引入的工具类
        tools: [],
