...
define(['../treat/timeToString','../treat/topBtnOperation','../treat/dataListConf'],function ($timetostring,topBtnOperation,dataListConf){
		...
		var btnConf = [{
            ...
            cols.forEach(function (fieldInfo) {
                ...
                switch (fieldInfo.showColValSet) {
                    case '3':
                        fieldKey = fieldInfo.moduleCode + "_" + fieldInfo.phyColumnName + "_ONAME";
                        break;
                    case '4':
            ...
            for(var key in cols){
                ...
                if(record != null && record.ID != null){
                    ...
                    if(isOwnRelated && moduleRelatedMap[colCode] != null){
                        ...
                        colVal = record[relatedModule.relModuleCode + relatedModule.showColumnCode + "_RNAME"]
                    } else{
                        if (record[col.moduleCode + col.phyColumnName + "_DNAME"] != null) {
                            colVal = record[col.moduleCode + col.phyColumnName + "_DNAME"];
                        } else if (record[col.moduleCode +  col.phyColumnName + "_RNAME"] != null) {
                            colVal = record[col.moduleCode + col.phyColumnName + "_RNAME"];
                        } else if (record[col.moduleCode + col.phyColumnName + "_ONAME"] != null) {
                            colVal = record[col.moduleCode + col.phyColumnName + "_ONAME"];
                        } else if (record[col.moduleCode + col.phyColumnName + "_UNAME"] != null) {
                            colVal = record[col.moduleCode + col.phyColumnName + "_UNAME"];
                        } else if (record[col.moduleCode + col.phyColumnName + "_FNAME"] != null) {
                            colVal = record[col.moduleCode + col.phyColumnName + "_FNAME"];
                        } else if (record[col.moduleCode + col.phyColumnName + "_IMGNAME"] != null) {
                            colVal = record[col.moduleCode + col.phyColumnName + "_IMGNAME"];
                        } else if (record[col.moduleCode + col.phyColumnName + "_SDNAME"] != null) {
                            colVal = record[col.moduleCode + col.phyColumnName + "_SDNAME"];
                        } else {
                            colVal = record[col.moduleCode + "_" + col.phyColumnName];
                        }
                    }
           // console.log(record,record[col.moduleCode + "_" + col.phyColumnName + "_DNAME"])
                    //根据规则对相应的值进行处理
