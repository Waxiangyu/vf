 ...
 */
define(['../treat/timeToString','../treat/topBtnOperation','../treat/gridBtnOperation'],function ($timetostring,topBtnOperation,gridBtnOperation) {

    // viewId = $_GET['viewId'];

    //顶部按钮组件的数据提取
    function extractTopBtn (btnList,gridApi) {
                ...
                if (btndata.operationPosition == '0') {
                    if (btndata.status == '0') {
                        var list ={
                            ...
                            events:topBtnOperation(btndata,gridApi),
                            iconEvents:{
    ...
    function exportFunc(viewId,gridApiConf,gridApi) {
        ...
        gridFieldInfo = {};
        //顶部按钮
        topBtn = extractTopBtn(btnList,gridApi);
        ...
        gridConf = {
            ...
            filtration: function (data, callback) {
                ...
                $FRAME.server({
                    serverType:'api',
                    method: 'POST',
                    url: 'babygridStructure'  //真实数据详情页面子数据
                }).fail(function (res) {
                    callback({});
