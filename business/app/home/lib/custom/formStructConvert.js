...
define(function () {
    ...
    function fieldConvert(fieldInfo, commConf, recordColumns) {
        ...
        switch (fieldInfo.showType) {
            ...
            case '1':
                ...
                var modelInfo=null;
                //检查是否是外键
                if (fieldInfo.colMark !== "foreignkey" || !(modelInfo=commConf.modelInfo[fieldInfo.columnCode] )) break;
        ...
        //检查是否隐藏字段
        if(commConf.showIds.indexOf(fieldId) == -1) fieldRes.hidden=true;
    ...
    //对外提供的接口
    return function exports(res, getType) {
        ...
        if(viewInfo.viewGroupList && viewInfo.viewGroupList.length){
            viewInfo.viewGroupList.forEach(function (groupList) {
                ...
                groupList.colList.forEach(function (fieldInfo) {
                    groupField.push(commConf.fieldLayoutMaps[fieldInfo.id]=fieldConvert(fieldInfo, commConf,res.recordColumns))
                })
            ...
            (res.columnList||[]).forEach(function (fieldInfo) {
                ...
                //字段数据转换
                resultFormLayout.push(commConf.fieldLayoutMaps[fieldInfo.id]=fieldConvert(fieldInfo, commConf,res.recordColumns));
            });


        }

        var tabConf=(getType && batchAddInfo(commConf,resultFormLayout))||{};
